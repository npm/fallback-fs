...
function overrideNode (handler) {
  ...
  function overrideAPISync (module, name, arg) {
    ...
    module[name] = function () {
      try {
        ...
        const resolved = handler.resolve(p)
        if (!resolved) {
        ...
        const stat = handler.statSync(resolved, true)
        if (!stat) {
          return notFoundError(p)
        }
  ...
  patchFn('lstatSync', lstatSync => p => {
    try {
      ...
      if (!resolved) {
        ...
        const stats = handler.statSync(resolved)
        if (!stats) {
          return notFoundError(p)
        }
        return stats
  ...
  patchFn('lstat', lstat => (p, cb) => {
    return lstat(p, (err, stats) => {
      ...
      handler.stat(resolved).then(newStats => {
        if (newStats) {
  ...
  patchFn('access', access => (p, mode, cb) => {
    ...
    access(p, mode, (err) => {
      ...
      if (resolved.isDir) {
        ...
        handler.stat(resolved).then(stat => {
          if (!stat) {
            return notFoundError(p, cb)
          }
  ...
  patchFn('accessSync', accessSync => (p, mode) => {
    ...
    try {
      ...
      if (!resolved) {
        ...
        const stat = handler.statSync(resolved)
        if (!stat) {
          return notFoundError(p)
        }
  ...
  patchFn('readFile', readFile => (p, options, cb) => {
    ...
    readFile(p, options, (err, data) => {
      ...
      handler.read(resolved).then(data => {
        if (encoding) {
  ...
  patchFn('readFileSync', readFileSync => (p, options) => {
    try {
      ...
      const { encoding } = options
      const data = handler.readSync(resolved)
      if (encoding) {
  ...
  patchFn('open', open => (p, flags, mode, cb) => {
    ...
    open(p, flags, mode, (err, fd) => {
      ...
      handler.stat(resolved, true).then(stat => {
        if (!stat) {
          return notFoundError(p)
        } else if (flags === 'r') {
  ...
  patchFn('internalModuleReadJSON', internalModuleReadJSON => p => {
    ...
    try {
      return handler.readSync(resolved).toString('utf8')
    } catch (err) {
      return undefined
    }
  }, process.binding('fs'), {})
